...
export const getAllStudents = async (req: CustomRequest<StudentQuery>, res: Response) => {
  try {
    const requestedBy = req.user?.role === Role.educator 
    if(!requestedBy) {
      return res.status(400).json({success: false, message: "Access is denied" });
    }
    ...
    return res.status(200).json({success: true, message: "Students fetched successfully", students, count });
  } catch (error) {
   console.error("Error in fetching students:", error);
    return res.status(400).json({ success: false, message: "Failed to fetch students" });
  } finally {
    await prisma.$disconnect();
  }
};